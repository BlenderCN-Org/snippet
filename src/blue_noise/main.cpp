...
/*
TODO:
- Mitchell’s Best Candidate Algorithm を理解する
- WhiteNoiseを書き出すようにする
- Void and Cluster を理解する
- Filtering White Noiseを理解する
- DFTイメージを作成できるようにする
*/
...
public:
    ...
    {
        const int32_t bpp = 1;
        stbi_write_png(fileName, width_, height_, bpp, pixel_.data(), width_*bpp);
    }
...
public:
    PointSet() {}
    float calcNearlest(const Point& point)
    {
        float nearSqr = std::numeric_limits<float>::infinity();
        for (const auto& p : points_)
        {
            const float dx = std::fabsf(p.x - point.x);
            const float dy = std::fabsf(p.y - point.y);
            const float dwx = std::min(dx, 1.0f - dx);
            const float dwy = std::min(dy, 1.0f - dy);
            const float distSqr = dwx * dwx + dwy * dwy;
            if (distSqr < nearSqr)
            {
                nearSqr = distSqr;
            }
        }
        return std::sqrtf(nearSqr);
    }
...
//  Mitchell’s Best Candidate Algorithm
void genBlueNoiseMitchell(int32_t numSample, int32_t m)
{
    std::mt19937 rng(0x123);
    std::uniform_real_distribution<float> dist;
    const auto genRandomPoint = [&]()
    {
        Point p;
        p.x = dist(rng);
        p.y = dist(rng);
    ...
    // 最初の1点はランダム
    PointSet pointSet;
    pointSet.add(genRandomPoint());

    //
    for (int32_t sn=1;sn<numSample;++sn)
    {
        // 
        const int32_t numCandidate = sn * m;
        float distFarest = 0.0f;
        Point cpp;
        for (int32_t ci=0;ci<numCandidate;++ci)
        {
            const Point cp = genRandomPoint();
            const float dist = pointSet.calcNearlest(cp);
            if (distFarest < dist)
            {
                cpp = cp;
                distFarest = dist;
    ...
    {
        const int32_t px = int32_t(point.x * float(image.width()));
        const int32_t py = int32_t(point.y * float(image.width()));
        image.setPixel(px, py, 0xFF);
    }
    image.write("./mitchell.png");
}
